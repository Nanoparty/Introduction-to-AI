...
def remainder(examples,attrName,attrValues,className):
    ...
    classcounts = cc.values()
    res = 0
    for v in attrValues:
        pertinentExamples = getPertinentExamples(examples,attrName,v)
        cc2=getClassCounts(pertinentExamples, className)
        kclasscounts = cc2.values()
        ent =setEntropy(kclasscounts)
        num=float(sum(kclasscounts))/sum(classcounts)
        res +=num* ent
    return res
...
def infoGain(examples,attrName,attrValues,className):
    ...
    return setEntropy(classcounts) - rem
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for v in attrValues:
        pertinentExamples = getPertinentExamples(examples,attrName,v)
        cc = getClassCounts(pertinentExamples, className)
        classcounts = cc.values()
        avg = float(len(pertinentExamples))/len(examples)
        g += avg * giniIndex(classcounts)
    if g == 0:
        return sys.maxint
    return 1.0/g
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for value in attributeValues[best]:
        ...
        if len(pertinentExamples) == 0:
            child = Node(defaultLabel)
            
            root.children[value] = child
            #print "no pertinent examples case hit"
        else:
            root.children[value] = makeSubtrees(newRemainingAttributes, pertinentExamples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc)
    return root
