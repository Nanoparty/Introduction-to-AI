    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
          ...
          if emissionModel[dist] > 0:
              ...
              sEmission = emissionModel[dist]

              sBeliefs = self.beliefs[p]

              allPossible[p] = sBeliefs * sEmission

        if noisyDistance is None:
            eBelief = util.Counter()
            ePos = 2 * self.index -1
            eBelief[(ePos, 1)] = 1
            allPossible = eBelief
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            gPos = self.setGhostPosition(gameState, oldPos)
            newPosDist = self.getPositionDistribution(gPos)

            for newPos, prob in newPosDist.items():
                bPos = self.beliefs[oldPos] * prob
                allPossible[newPos] = allPossible[newPos] + bPos
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for pos in self.legalPositions:
            ...
            td = util.manhattanDistance(pacmanPosition, pos)
            if emissionModel[td] > 0:
                newBeliefs[pos] = emissionModel[td] * beliefs[pos]
        ...
        if newBeliefs.totalCount() != 0:
            ...
            for i in range (0, parts):
                newB = util.sampleFromCounter(newBeliefs)
                self.particles.append(newB)
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            gp = self.setGhostPosition(gameState, pos)
            newPosDist = self.getPositionDistribution(gp)
            sp = util.sampleFromCounter(newPosDist)
            newParticles.append(sp)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        tp = [tuple([random.choice(self.legalPositions) for i in range(self.numGhosts)]) for j in range(self.numParticles)]
        self.particles = tp
        ...
        for pos in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != 999:
                    ...
                    partW *= emissionModels[i][trueDistance]
            tpos = tuple(pos)
            distrib[tpos] += partW

        if sum(distrib.values()) != 0:
            newParticles = []
            for i in range(self.numParticles):
                sample = util.sample(distrib)
                newParticle = []
                ...
                for j in range(self.numGhosts):
                    if noisyDistances[j] != 999:
                        ...
                        val = (2*j + 1, 1)
                        newParticle.append(val)

                np = tuple(newParticle)
                newParticles.append(np)
            self.particles = newParticles
        else:
            self.initializeParticles()
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            val5 = setGhostPositions(gameState, newParticle)
            newParticle = [util.sample(getPositionDistributionForGhost(val5, i, self.ghostAgents[i])) for i in range(self.numGhosts)]
            "*** END YOUR CODE HERE ***"
